...
// Supported authenicators
authenticators.SNOWFLAKE = 'snowflake';
authenticators.EXTERNAL_BROWSER = 'externalbrowser';
...
function consolidateAuthenticator(options)
{
  if (Util.exists(options.authenticator) && Util.string.isNotNullOrEmpty(options.authenticator))
  {
    if (![authenticators.SNOWFLAKE, authenticators.EXTERNAL_BROWSER].includes(options.authenticator))
    {
      Errors.checkArgumentValid(
        false, ErrorCodes.ERR_CONN_CREATE_INVALID_AUTHENTICATOR);
    }
  }
  else
  {
    options.authenticator = authenticators.SNOWFLAKE;
  }
}
...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo)
{
  ...
  if (validateCredentials)
  {
    ...
    // check for invalid authenticator
    if (Util.exists(options.authenticator))
    {
      Errors.checkArgumentValid(Util.isString(options.authenticator),
        ErrorCodes.ERR_CONN_CREATE_INVALID_AUTHENTICATOR);
    }

    consolidateAuthenticator(options);

    if (options.authenticator === authenticators.SNOWFLAKE)
    {
