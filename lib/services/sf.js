...
StateConnecting.prototype.enter = function (context)
{
  ...
  if (this.connectionConfig.authenticator === 'externalbrowser') {
    this.webBrowserAuth({});
  }
  else
  {
 ...
 * @inheritDoc
 */
StateConnecting.prototype.snowflakeAuth = function (options)
{
  var err = options.err;
 ...
 * @inheritDoc
 */
StateConnecting.prototype.webBrowserAuth = function (options)
{
  var context = this.context;
  const parent = this;

  // issue an authenticator request to get SSO Url
  const sendSsoUrlRequest = function (redirectPort)
  {
    const request = parent.createUnauthenticatedRequest({
      method: 'POST',
      ...
      {
        // open default browser with ssoUrl
        if (!err)
        {
          Open(body.data.ssoUrl, { wait: false });
        }
        else
        {
          // we're now disconnected
  ...
  const server = Http.createServer(function(req, res) {
    ...
    if (newSamlToken)
    {
      parent.tokenInfo.update({ 'token': newSamlToken });

      res.writeHead(200, {'Content-Type': 'text/plain'});
      res.write("Your identity was confirmed and propagated to Snowflake Node.js connector.");
      res.write("You can close this window now and go back where you started from.");

      // close the server we don't need it any more
      server.close();
  ...
  server.listen(0, function() {
    sendSsoUrlRequest(server.address().port);
  });
}
